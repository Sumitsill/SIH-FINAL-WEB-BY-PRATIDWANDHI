...
import { Trophy, Users, ShieldCheck, Eye, EyeOff, Building2, HeartHandshake, User, Mail, Lock } from "lucide-react";
...
const roleConfigs: Record<RoleType, RoleConfig> = {
    athlete: {
        title: "auth.roles.athlete.signup_title",
        icon: User,
        description: "auth.roles.athlete.signup_desc",
        gradient: "from-blue-900 via-cyan-800 to-indigo-900",
        accentColor: "text-cyan-400"
    },
    coach: {
        title: "auth.roles.coach.signup_title",
        icon: Users,
        description: "auth.roles.coach.signup_desc",
        gradient: "from-green-900 via-emerald-800 to-teal-900",
        accentColor: "text-emerald-400"
    },
    academy: {
        title: "auth.roles.academy.signup_title",
        icon: Building2,
        description: "auth.roles.academy.signup_desc",
        gradient: "from-purple-900 via-indigo-800 to-violet-900",
        accentColor: "text-purple-400"
    },
    sponsor: {
        title: "auth.roles.sponsor.signup_title",
        icon: HeartHandshake,
        description: "auth.roles.sponsor.signup_desc",
        ...
        try {
            const { data, error } = await supabase.auth.signUp({
                ...
                options: {
                    data: {
                        full_name: fullName,
                        role: currentRole, // athlete | coach | academy | sponsor
                    },
                },
            });
        ...
        <div className={`min-h-screen flex items-center justify-center bg-gradient-to-br ${config.gradient} relative overflow-hidden transition-colors duration-500`}>
            ...
            <div className="w-full max-w-md bg-slate-900/80 backdrop-blur-md p-8 rounded-2xl shadow-2xl border border-white/10 relative z-10">
                ...
                <form onSubmit={handleSignup} className="space-y-5">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder={t('auth.common.create_password_placeholder')}
                    ...
                    <button
                        type="submit"
                        className={`w-full py-3.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold text-lg shadow-lg shadow-cyan-500/20 hover:shadow-cyan-500/40 hover:scale-[1.02] transition-all duration-200`}
                    >
                    ...
                    <div className="relative my-6">
                        ...
                        <div className="relative flex justify-center text-sm">
                            <span className="px-2 bg-slate-900 text-gray-400">{t('auth.common.or_signup')}</span>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <span>{t('auth.common.google_signup')}</span>
                    </button>
                </form>
                ...
                <div className="mt-8 bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 flex items-start space-x-3">
                    ...
                    <p className="text-yellow-200/80 text-xs leading-relaxed">
                        <span className="font-semibold text-yellow-400">{t('auth.common.fair_play')}</span> {t('auth.common.verify_msg')}
                    </p>
                </div>
            </div>
        </div>
