import React, { useState, useEffect } from 'react'; // Force HMR update
...
export default function VoiceAssistant() {
  ...
  useEffect(() => {
    ...
    script.async = true;
    script.src = 'https://backend.omnidim.io/web_widget.js?secret_key=2f0da70268bbc771cc176286eeeb6d55';
  ...
  const toggleSpeech = () => {
    if (isSpeaking) {
      ...
      speak('shram is ready. How can I help you today?');
    }
  };
    ...
    <div className="fixed bottom-6 right-6 z-50">
      ...
      <div className="flex flex-col space-y-3">
        ...
        <button
          ...
          className={`w-16 h-16 rounded-full shadow-lg transition-all duration-300 transform hover:scale-110 flex items-center justify-center ${isListening
        ...
        <button
          ...
          className={`w-12 h-12 rounded-full shadow-lg transition-all duration-300 transform hover:scale-110 flex items-center justify-center ${isSpeaking
      ...
      {/* Status Indicator */}
      <div className={`absolute -top-2 -right-2 w-4 h-4 rounded-full ${isWidgetLoaded ? 'bg-green-500' : 'bg-yellow-500'
        } shadow-lg`}></div>
    </div>
  );
}